...
This repository contains files, scripts, and notebooks to reproduce the models and results for the ADME public dataset presented in "Uncertainty quantification in molecular machine learning for property predictions under data shifts" by Parrondo-Pizarro et al. It also includes Jupyter notebooks to generate the figures included in the manuscript for public data.

### Conda environments

Two YAML files to create the Conda environments to run Chemprop for model training and evaluation (`chemprop-env.yml`) and execute UNIQUE pipeline to perform UQ benchmarking (`unique-env.py`) are included.
...
* `Figures_PublicData.ipynb`: Reproduces the figures of the manuscript for public data.
